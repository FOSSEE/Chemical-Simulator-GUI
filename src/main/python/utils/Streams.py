...
class MaterialStream():
    ...
    def OM_Flowsheet_Equation(self, addedcomp, method):
        """Generate the Modelica equation section for MaterialStream"""
        self.OM_data_eqn = ''
        self.comp_count = len(addedcomp)

        # Always update eqn_dict for compatibility
        self.eqn_dict = {}
        self.get_min_eqn_values()

        # --- 1️⃣ Pressure ---
        P_val = self.variables['P']['value']
        if P_val is not None:
            self.OM_data_eqn += f"{self.name}.P = {P_val};\n"

        # --- 2️⃣ Temperature ---
        T_val = self.variables['T']['value']
        if T_val is not None:
            self.OM_data_eqn += f"{self.name}.T = {T_val};\n"

        # --- 3️⃣ Flowrate (Mixture Molar Flow) ---
        Fp_val = self.variables['F_p[1]']['value']

        if Fp_val is not None:
            self.OM_data_eqn += f"{self.name}.F_p[1] = {Fp_val};\n"

        # --- 4️⃣ Mole fractions (x_pc[1,:]) ---
        mole_fracs = []
        for i in range(1, len(self.compound_names) + 1):
            key = f"x_pc[1,{i}]"
            val = self.variables.get(key, {}).get('value', None)
            if val is not None:
                mole_fracs.append(str(val))

        if mole_fracs:
            formatted = ", ".join(mole_fracs)
            self.OM_data_eqn += f"{self.name}.x_pc[1, :] = {{{formatted}}};\n"

        # --- 5️⃣ Include any extra eqn_dict (fallbacks) ---
        for key, value in self.eqn_dict.items():
            if key not in ['P', 'T', 'F_p[1]', 'x_pc[1,:]']:
                self.OM_data_eqn += f"{self.name}.{key} = {value};\n"
    ...
    def disableInputDataTab(self, dockwidget):
        ...
        # Safe access helper
        def safe_set_text(widget, key, default=0):
            if key in self.variables:
                widget.setText(str(round(float(self.variables[key]['value']),4)))
            else:
                widget.setText(str(default))
                print(f"[DEBUG] Key not found: {key}")
            widget.setDisabled(True)

        safe_set_text(dockwidget.input_dict['P'], 'P')
        safe_set_text(dockwidget.input_dict['T'], 'T')
        safe_set_text(dockwidget.input_dict['MolFlow'], 'F_p[1]')
        ...
        # Disable x_pc values safely
        for index, k in enumerate(dockwidget.x_pclist):
            key_name = f"x_pc[1,{index+1}]"
            safe_set_text(k, key_name)
