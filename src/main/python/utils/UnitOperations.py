...
class UnitOperation():
    ...
    def OM_Flowsheet_Equation(self):
        ...
        if hasattr(self, 'mode') and self.mode:
            self.OM_data_eqn += f'{self.name}.{self.mode} = {self.mode_val};\n'
    ...
    def OM_Flowsheet_Equation(self):
        ...
        if self.output_stms:
            ...
            for strm in self.output_stms.values():
                if hasattr(strm, 'name') and strm.name:
                    if strcount == 1:
                        self.OM_data_eqn += f'connect({strm.name}.In, {self.name}.Out);\n'
                    else:
                        self.OM_data_eqn += f'connect({strm.name}.In, {self.name}.Out{strcount});\n'
                    strcount += 1

        # Key component specifications (only if available)
        if 'HKey_x_pc' in self.variables and 'LKey_x_pc' in self.variables:
            self.OM_data_eqn += (
                f"{self.name}.x_pc[2, {self.name}.HKey] = {self.variables['HKey_x_pc']['value']};\n"
            )
            self.OM_data_eqn += (
                f"{self.name}.x_pc[3, {self.name}.LKey] = {self.variables['LKey_x_pc']['value']};\n"
            )
...
class CompoundSeparator(UnitOperation):
    ...
    def OM_Flowsheet_Equation(self):
        ...
        # Separator values
        if 'SepVal_c' in self.variables:
            val = self.variables['SepVal_c']['value']
            if isinstance(val, list):  # vector case
                SepVal = "{" + ",".join(map(str, val)) + "}"
            else:  # scalar case
                SepVal = str(val)
            self.OM_data_eqn += f'{self.name}.SepVal_c = {SepVal};\n'
...
class Flash(UnitOperation):
    ...
    def OM_Flowsheet_Equation(self):
        ...
        strcount = 1
        for strm in self.output_stms.values():
            # If unit has multiple outputs, use Out1, Out2...
            # If it has only single output, just use 'Out'
            out_port = f"Out{strcount}" if len(self.output_stms) > 1 else "Out"
            self.OM_data_eqn += f'connect({strm.name}.In, {self.name}.{out_port});\n'
            strcount += 1
